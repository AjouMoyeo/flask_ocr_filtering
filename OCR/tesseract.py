...
file_list = os.listdir('image_localized/')
for file in file_list:
    image = cv2.imread('image_localized/' + file)
...
# Simple image to string
    text = pytesseract.image_to_string(Image.open(filename), lang='kor+eng', config = '--psm 6')
    os.remove(filename)
