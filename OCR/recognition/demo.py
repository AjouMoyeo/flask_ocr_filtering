...
def demo(opt):
    ...
    for i in range(len(prediction)):
        ...
        for j in range(len(prediction[i])):
            tmp += str(prediction[i][j])
        lined_prediction.append(tmp)
        tmp = ''
    for i in range(len(x)):
        lined_prediction[i] = lined_prediction[i].replace('\n', '')
        lined_prediction[i] = lined_prediction[i].replace(' ', '')
        print(x[i], "       ", lined_prediction[i])
    with open(f'result.txt', 'w') as file:
        for i in range(len(prediction)):
            file.write(str(x[i]) + " " + str(lined_prediction[i]) + '\n')
